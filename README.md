...
- [5. Operacions](#5-operacions)
  ...
  * [5.7 Consulta amb filtres de representants per poderdant](#57-consulta-amb-filtres-de-representants-per-poderdant)
      - [Peticio](#peticio-5)
      - [Resposta](#resposta-6)      
  * [5.8 Validacio](#58-validacio)
      - [Peticio](#peticio-6)
      - [Resposta](#resposta-7)
  * [5.9 Alta o modificacio](#59-alta-o-modificacio)
      - [Peticio](#peticio-7)
      - [Resposta](#resposta-8)
  * [5.10 Consulta de cataleg](#510-consulta-de-cataleg)
      - [Peticio](#peticio-8)
      - [Resposta](#resposta-9)
  * [5.11 Consulta de families](#511-consulta-de-families)
      - [Peticio](#peticio-9)
      - [Resposta](#resposta-10)
  * [5.12 Consulta de familia](#512-consulta-de-familia)
      - [Peticio](#peticio-10)
      - [Resposta](#resposta-11)
  * [5.13 Consulta de tramits](#513-consulta-de-tramits)
      - [Peticio](#peticio-11)
      - [Resposta](#resposta-12)
  * [5.14 Consulta administracio](#514-consulta-administracio)
      - [Peticio](#peticio-12)
      - [Resposta](#resposta-13)
  * [5.15 Consulta d'administracions](#515-consulta-d-administracions)
      - [Peticio](#peticio-13)
      - [Resposta](#resposta-14)
  * [5.16 Descarrega de documents](#516-descarrega-de-documents)
...
|A l'hora de realitzar la vostra integració amb Representa, tingueu present les recomanacions sobre **Protecció de Dades** recollides al document https://www.apdcat.cat/content/dam/aem-apdcat/03-documentacio/documents/guiadesenvolupadors/GUIA-PDDD.pdf de l'APDCAT. |
...
|ambitRepresentacio | Element per definir l'ambit de la representació. Segons el _tipusRepresentacio_ s'inclou l'element _administracio_ i/o _tramit_ i/o _capacitats_|
...
|descripcio | Descripció del tramit |
...
* `VALIDA` >> Representació en estat operatiu, que ha estat acceptada pel representant o pel poderdant, o bé validada pel validador. (Quan es faci una consulta de Validació, una representació en aquest estat respondrà afirmativament)
* `PENDENT_VALIDACIO` >> S'ha aportat documentació que cal revisar i validar per poder canviar l'estat a VALIDA o DENEGADA. Temporalment, també es pot passar la representació a EN_VALIDACIÓ, per indicar que està en procés de revisió. (Quan es faci una consulta de Validació, una representació en aquest estat no respondrà afirmativament)
* `EN_VALIDACIO` >> S'està revisant la documentació adjunta. Un cop validada es passarà a VALIDA, DENEGADA o altre cop a PENDENT_VALIDACIO. (Quan es faci una consulta de Validació, una representació en aquest estat no respondrà afirmativament)
...
* `PENDENT_ACCEPTACIO` >> El poderdant o representant ha realitzat una representació sense documentació a validar i cal que l'altra part l’accepti explícitament per passar l'estat a VALIDA. Qualsevol de les dues parts però, pot decidir donar-la de baixa, tot passant a RENUNCIADA o REVOCADA (segons qui faci l'acció). Passats 40 dies en aquest estat, si no ha estat acceptada o donada de baixa (revocació o renúncia), automàticament passa a anul·lada. (Quan es faci una consulta de Validació, una representació en aquest estat no respondrà afirmativament)
* `DENEGADA` >> Un cop revisada la documentació adjunta a la inscripció, el validador pot denegar la representació.
* `EXPIRADA` >> Una representació expira quan abans ha estat VALIDA, PENDENT_ACCEPTACIO o PENDENT_VALIDACIO, però en la data actual està fora del seu periode de vigència.
* `RENUNCIADA` >> El representant renuncia a la representació.
* `REVOCADA` >> El poderdant revoca la representació.
* `ANULADA` >> Estat excepcional en que l'administració pública decideix donar de baixa una representació, ja sigui automàticament pel sistema per què han passat 40 dies sense ser acceptada o 90 dies sense ser validada, o bé manualment pel funcionari, si hi ha una errada en la mateixa (p.ex. NIF incorrecte, nom incorrecte, documentació adjunta no coincident, etc.) Una representació pot passar a aquest estat en qualsevol moment de la seva vida útil.

<p align="center">
<img align="center" src="img/Estats_Representa.png" />
</p> 
